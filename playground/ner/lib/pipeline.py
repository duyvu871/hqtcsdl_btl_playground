"""NER pipeline — hybrid, validator, full LLM modes."""
...
from lib.rules import Mention, RuleResult, extract_rules, looks_crypto_related

Mode = Literal["hybrid", "validator", "full"]
...
def _merge_mentions(primary: list[Mention], secondary: list[Mention]) -> list[Mention]:
    seen: set[str] = set()
    out: list[Mention] = []
    for mention in primary + secondary:
        if mention.coin_id in seen:
            continue
        seen.add(mention.coin_id)
        out.append(mention)
    return out
