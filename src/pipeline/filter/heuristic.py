...
Kiểm tra nhanh trước ML: text rỗng, min_likes, engagement ratio, pump regex.
News (source=news) bỏ qua hầu hết rule — chỉ check text không rỗng.
"""
...
# Regex pump/shill — DROP tại L1 (TC-01)
...
class HeuristicConfig:
    """Tham số L1 — 0/false = tắt rule tương ứng."""
    min_likes: int = 0
    min_text_len: int = _MIN_TEXT_LEN
    max_per_author: int = 0  # 0 = tắt cap trong batch
    min_engagement_ratio: float = 0.0
    max_engagement_ratio: float = 0.0  # >0 flag shill bất thường
    skip_news: bool = True
...
def check_heuristic(
    ...
    *,
    ...
    """L1: rule-based filter. News chỉ check text rỗng nếu skip_news=True."""
    ...
    if config.max_per_author > 0 and author_counts is not None:
        if author_counts.get(author, 0) >= config.max_per_author:
